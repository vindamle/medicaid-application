...
def track_untrack_resident(request):
    ...
    if track == "true":
        ...
        # Audit Log
        Snowden.objects.create(user = request.user,table_name = resident._meta.verbose_name,row_id = resident_id,column_name = column,old_value = getattr(resident,column) if getattr(resident,column) is not None else "None",new_value = "False",log_ip = request.META.get('REMOTE_ADDR'),date = datetime.now())
        resident.tracking_status = False

        # Audit Log
        Snowden.objects.create(user = request.user,table_name = resident._meta.verbose_name,row_id = resident_id,column_name = column,old_value = getattr(resident,column) if getattr(resident,column) is not None else "None",new_value = "True",log_ip = request.META.get('REMOTE_ADDR'),date = datetime.now())
        resident.dismiss = True
        resident.save()
